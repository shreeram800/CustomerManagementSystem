...
public class CustomerService {
    ...
    public List<Customer> getWithEnd(String name){
        if(name==null){
            throw  new DuplicateNotFoundException("Empty name.");
        }
        return  customerDao.selectAllCustomers().stream().filter(e->e.getName().toLowerCase(Locale.ROOT).endsWith(name)).toList();
    }
}
